...
`$harness` 또는 `$codex-harness:harness`를 명시적으로 호출하면 일반 콘텐츠 작업을 함께 요청해도 **현재 대상 프로젝트에 지속적으로 사용할 하네스를 기본으로 구축하거나 갱신**한 뒤 해당 작업을 수행합니다. 읽기 전용으로 처리하거나 하네스를 만들지 말라는 명시적 요청이 있으면 그 제한을 우선합니다. 에이전트 정의는 대상 프로젝트의 `.codex/agents/*.toml`에, 재사용 스킬은 `.agents/skills/`에 둡니다. 필요에 따라 `AGENTS.md` 포인터, 프로젝트 설정, 실행 기록도 만듭니다. Codex를 대상 프로젝트에서 시작하면 네이티브 에이전트도 플러그인 캐시가 아닌 그곳에서 작업합니다. 플러그인 설치만으로는 프로젝트 파일이 생기지 않습니다. 미리 정의된 5개 역할의 팀은 선택 사항인 [프로젝트 설치 도구](#다른-프로젝트에-설치하기-선택)로 설치합니다.

에이전트를 사용하기 전에 `codex -C /absolute/path/to/project`로 대상 프로젝트에서 Codex를 시작하세요. 네이티브 spawn에는 별도 `cwd` 인자가 없고 부모 프로젝트 세션의 작업 디렉터리를 따릅니다. 다른 위치에서 시작한 세션은 명시적 workdir로 대상에 부트스트랩 파일을 쓸 수 있지만, 에이전트 실행은 대상 프로젝트에서 새 세션을 열어 진행하세요. 보호된 디렉터리의 쓰기 오류는 [빠른 시작 문제 해결](docs/quickstart.md#troubleshooting)을 참고하세요.
...
설치 도구는 `harness` 스킬과 기본 에이전트를 복사하고, 기존 내용을 보존하면서 지원하는 형식의 프로젝트 설정과 `AGENTS.md` 포인터를 병합합니다. 이 미리 정의된 구성은 선택 사항이며, 명시적인 스킬 호출만으로도 프로젝트에 맞는 하네스를 구축합니다. 충돌이 보고되면 해당 내용을 확인하세요. 기본값을 추가해야 하는 `agents = { ... }` 인라인 테이블은 `[agents]` 형식으로 펼쳐야 할 수 있으며, 이 경우 설치 도구는 파일을 쓰기 전에 진단을 반환합니다. 사용자 전역 Codex 설정은 변경하지 않습니다. 설치 후 대상 프로젝트를 신뢰하는 로컬 프로젝트로 열고 새 Codex 세션을 시작하세요.

[Codex 플러그인 매니페스트](.codex-plugin/plugin.json)는 표준 `skills/` 디렉터리에서 스킬을 패키징하며, [저장소 마켓플레이스](.agents/plugins/marketplace.json)를 통해 `codex-harness@codex-harness`로 설치할 수 있습니다. 플러그인 설치 자체는 프로젝트 파일을 생성하지 않습니다. 명시적인 스킬 호출은 프로젝트에 맞는 하네스를 구축하고, 위의 설치 도구는 선택적인 기본 구성을 제공합니다.
...
아래 셸 예제는 명시적인 `$harness` 호출이나 선택적 프로젝트 설치 도구가 만드는 경로를 사용합니다. 플러그인만 설치하고 아직 호출하지 않았다면 보조 스크립트는 설치된 스킬 디렉터리에 있습니다.
...
**자동 테스트 94개가 통과**했습니다. 별도로 **Codex CLI 0.153.4에서 네이티브 커스텀 워커 2개와 독립 QA 에이전트**를 사용해 파일 작성, 동료 통신 기록, 차단 상태의 완료 거절, 부분 재개, 성공 결과 재사용, 최종 테스트 통과를 확인했습니다. [검증 기록](docs/verification.md)에 근거와 범위, 이전 읽기 전용 실행과 ephemeral 세션에서 관찰한 문제를 남겼습니다. 다른 클라이언트, 샌드박스의 강제 적용, 성능 향상까지 검증한 것은 아닙니다. 사용 환경에서는 위의 실제 실행 검증 명령으로 확인할 수 있습니다.
