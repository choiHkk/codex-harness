...
1. Initialize run first from plan.template.json. Probe native availability by spawning only the first harness_worker into standby, with no product reads or writes. Inspect the actual spawn result and real child ID before any further spawn or run.py start. If spawning fails with `collab spawn failed: no thread with id` or returns no usable child ID, stop further spawning, preserve the exact error and any known CLI version/session mode in _workspace/native-failure.md, and report native smoke failure. Do not retry this missing-parent-thread error per role, invent IDs, substitute sequential parent work, or silently change session persistence. On success, reuse this SAME child as alpha and register it via run.py start; do not create an extra probe worker. Within actual native capacity, spawn the second harness_worker into standby, collect its real ID, register beta via run.py start, then give both their input.md and each other's IDs before waiting. You are not alone; each owns only its matching .py file. Tell them to read refreshed packets and preserve others' work. Have alpha ask beta a relevant contract question via actual native send (e.g. whether both functions use value() and whether beta has an input blocker); beta must answer with the observed mismatch and notify the parent. Record correlated question/answer and actual delivery observations with communication.py. The parent may relay/log if necessary but must actually deliver the exchange. Record sender/recipient as real native IDs and attach message IDs to native text. No fake heartbeat or invented delivery.
...
This is a bounded fixture: two 2-line modules and two small unit tests. Continue through the deliberate blocker using the checkpoint transition; no user confirmation is needed. This smoke specifically requires native delegation: if it is unavailable or a required check fails, report the failure honestly and leave evidence intact. Do not pass this smoke through sequential parent implementation; unstarted native tasks remain pending and later unperformed checks remain not_run.
"""
