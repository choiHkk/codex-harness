...
작업 완료와 네이티브 에이전트의 유휴·중단·종료는 서로 다른 사실이다. 새 네이티브 에이전트는 먼저 작업 지시를 기다리는 상태로 생성한다. 이 최초 지시에는 아직 제품 파일을 읽거나 쓰지 말고 최신 패킷을 기다리라고 명시한다. 실제 반환된 ID를 레지스트리에 연결한 뒤 본 작업을 전달한다. 다음 명령의 `ACTUAL_AGENT_ID`는 실제 도구가 반환한 값으로 바꾼다.
